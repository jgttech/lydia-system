...
export default useResponse(
    "Lexicon",
    async (Lexicon, response, root, { _id, entity }) => {
        ...
        try {
            ...
            if (lexicon == null)
            ...
            else if (await Lexicon.hasEntityWithLabel(lexicon, entity))
                await response.setError(...STATUS.LEXICON_ENTITY_EXISTS);
            else {
                ...
                if (!await Lexicon.hasEntityWithLabel(lexicon, entity))
                    await response.setError(...STATUS.LEXICON_ENTITY_UPDATE_FAILURE);
                else
